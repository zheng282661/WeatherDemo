...
function MediaStreamRecorder(mediaStream) {
	...
	this.save = function(file, fileName) {
		if (!file) {
			...
			ConcatenateBlobs(mediaRecorder.blobs, mediaRecorder.blobs[0].type,
	...
	this.pause = function() {
		...
		mediaRecorder.pause();
		console.log('Paused recording.', this.mimeType
	...
	this.resume = function() {
		...
		console.log('Resumed recording.', this.mimeType
				|| mediaRecorder.mimeType);
	};
...
function MultiStreamRecorder(mediaStream) {
	...
	this.start = function(timeSlice) {
		...
		audioRecorder.ondataavailable = function(blob) {
			...
			if (audioVideoBlobs[recordingInterval].video
					&& !audioVideoBlobs[recordingInterval].onDataAvailableEventFired) {
		...
		videoRecorder.ondataavailable = function(blob) {
			...
			if (audioVideoBlobs[recordingInterval].audio
					&& !audioVideoBlobs[recordingInterval].onDataAvailableEventFired) {
				audioVideoBlobs[recordingInterval].onDataAvailableEventFired = true;
				fireOnDataAvailableEvent(audioVideoBlobs[recordingInterval]);
			}
...
var IsOpera = false;
if (typeof opera !== 'undefined' && navigator.userAgent
...
function MediaRecorderWrapper(mediaStream) {
	...
	this.start = function(timeSlice, __disableLogs) {
		...
		if (!self.disableLogs && !__disableLogs) {
			console.log('Passing following params over MediaRecorder API.',
					recorderHints);
		}
		...
		if ('canRecordMimeType' in mediaRecorder
		...
		mediaRecorder.onerror = function(error) {
			if (!self.disableLogs) {
				if (error.name === 'InvalidState') {
					console
							.error('The MediaRecorder is not in a state in which the proposed operation is allowed to be executed.');
				} else if (error.name === 'OutOfMemory') {
					console
							.error('The UA has exhaused the available memory. User agents SHOULD provide as much additional information as possible in the message attribute.');
				} else if (error.name === 'IllegalStreamModification') {
					console
							.error('A modification to the stream has occurred that makes it impossible to continue recording. An example would be the addition of a Track while recording is occurring. User agents SHOULD provide as much additional information as possible in the message attribute.');
				} else if (error.name === 'OtherRecordingError') {
					console
							.error('Used for an fatal error other than those listed above. User agents SHOULD provide as much additional information as possible in the message attribute.');
...
function StereoAudioRecorderHelper(mediaStream, root) {
	...
	this.requestData = function() {
		...
		var leftBuffer = mergeBuffers(internalLeftChannel,
		...
		if (numChannels === 2) {
			var rightBuffer = mergeBuffers(internalRightChannel,
					internalRecordingLength); // bug fixed via #70,#71
	...
	if (context.createJavaScriptNode) {
		scriptprocessornode = context.createJavaScriptNode(bufferSize,
				numChannels, numChannels);
	} else if (context.createScriptProcessor) {
		scriptprocessornode = context.createScriptProcessor(bufferSize,
				numChannels, numChannels);
	} else {
...
function WhammyRecorderHelper(mediaStream, root) {
	this.record = function(timeSlice) {
		...
		console.log('video width/height', video.width || canvas.width, '*',
	...
	this.requestData = function() {
		...
		whammy.compile(function(whammyBlob) {
			...
			console.debug('video recorded blob size:',
					bytesToSize(whammyBlob.size));
		});
	...
	function drawFrames() {
		...
		if (!isOnStartedDrawingNonBlankFramesInvoked
				&& !isBlankFrame(whammy.frames[whammy.frames.length - 1])) {
	...
	function isBlankFrame(frame, _pixTolerance, _frameTolerance) {
		...
		var maxColorDifference = Math.sqrt(Math.pow(255, 2) + Math.pow(255, 2)
				+ Math.pow(255, 2));
		var pixTolerance = _pixTolerance && _pixTolerance >= 0
				&& _pixTolerance <= 1 ? _pixTolerance : 0;
		var frameTolerance = _frameTolerance && _frameTolerance >= 0
				&& _frameTolerance <= 1 ? _frameTolerance : 0;
		...
		var imageData = context2d.getImageData(0, 0, canvas.width,
				canvas.height);
		matchPixCount = 0;
		...
		for ( var pix = 0; pix < endPixCheck; pix += 4) {
			...
			var colorDifference = Math.sqrt(Math.pow(currentColor.r
					- sampleColor.r, 2)
	...
	function dropBlackFrames(_frames, _framesToCheck, _pixTolerance,
			_frameTolerance) {
		var localCanvas = document.createElement('canvas');
		localCanvas.width = canvas.width;
		localCanvas.height = canvas.height;
		...
		var endCheckFrame = (_framesToCheck && _framesToCheck > 0 && _framesToCheck <= _frames.length) ? _framesToCheck
				: _frames.length;
		var sampleColor = {
			r : 0,
			g : 0,
		...
		for ( var f = 0; f < endCheckFrame; f++) {
			...
			if (!doNotCheckNext
					&& maxPixCount - matchPixCount <= maxPixCount
							* frameTolerance) {
...
var Whammy = (function() {
	...
	function processInWebWorker(_function) {
		var blob = URL.createObjectURL(new Blob([
				_function.toString(),
				'this.onmessage =  function (e) {' + _function.name
						+ '(e.data);}' ], {
	...
	function whammyInWebWorker(frames) {
		function ArrayToWebM(frames) {
			...
			while (frameNumber < frames.length) {
				...
				var cluster = {
					...
					'data' : getClusterData(clusterTimecode, clusterCounter,
							clusterFrames)
		...
		function generateEBML(json) {
			...
			for ( var i = 0; i < json.length; i++) {
				...
				var zeroes = Math
						.ceil(Math.ceil(Math.log(len) / Math.log(2)) / 8);
				var sizeToString = len.toString(2);
				var padded = (new Array((zeroes * 7 + 7 + 1)
						- sizeToString.length)).join('0')
						+ sizeToString;
		...
		function makeSimpleBlock(data) {
			...
			var out = [ data.trackNum | 0x80, data.timecode >> 8,
					data.timecode & 0xff, flags ].map(function(e) {
		...
		function getStrLength(string, offset) {
			return parseInt(string.substr(offset + 4, 4).split('').map(
					function(i) {
						var unpadded = i.charCodeAt(0).toString(2);
						return (new Array(8 - unpadded.length + 1)).join('0')
								+ unpadded;
					}).join(''), 2);
		}
		...
		function doubleToString(num) {
			return [].slice.call(
					new Uint8Array((new Float64Array([ num ])).buffer), 0).map(
					function(e) {
						return String.fromCharCode(e);
					}).reverse().join('');
		}
